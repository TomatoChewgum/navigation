...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
    ...
    //1.查找激光到base_link的转换是否存在,不存在的添加到队列中,存在则从队列中查找到index
...
//        ROS_ERROR("pose\n");
    ...
    if(lasers_update_[laser_index])
    {
        ...
        //8.将laserScan数据转换为AMCLLaserData数据类型
